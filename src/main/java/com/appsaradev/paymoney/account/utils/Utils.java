...
public class Utils {
	...
	public static void sendEmail(String subject, User user) {
		String link = "http://localhost:8080/rest/createuser/verify?token=" + Utils.encodedBase64(user.getEmail()) + "";
	...
	public static boolean isExpire(String creationTime, String currentTime) {
		boolean isExpire = true;
		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
		try {
			Date create = format.parse(creationTime);
			Date curr = format.parse(currentTime);
			long timeRes = curr.getTime() - create.getTime();
			System.out.println(curr.getTime());
			System.out.println(create.getTime());
			System.out.println("Time after sub: " + timeRes);
			if (timeRes >= 30 * 24 * 3600) {
				isExpire = false;
			}
		} catch (Exception e) {
			System.out.println("isExpire: " + e.getMessage());
		}
		System.out.println("isExpire: " + isExpire);
		return isExpire;
	}

	public static String getEncodeFromString(String value) {
		String encode = null;
		try {
			encode = URLEncoder.encode(value, "UTF-8");
		} catch (Exception e) {
			System.out.println("getEncodeFromString: " + e.getMessage());
		}
		System.out.println("getEncodeFromString: " + encode);
		return encode;
	}

	public static String getDecoderFromString(String value) {
		String decode = null;
		try {
			decode = URLDecoder.decode(value, "UTF-8");
		} catch (Exception e) {
			System.out.println("getDecoderFromString: " + e.getMessage());
		}
		System.out.println("getDecoderFromString: " + decode);
		return decode;
	}

		...
		.ready(
				function($) {
					var formModal = $('.cd-user-modal'), formLogin = formModal
							...
							.find('.cd-form-bottom-message a'), mainNav = $('.main-nav');
...
/*					formLogin.find('input[type="submit"]').on(
							'click',
							function(event) {
								event.preventDefault();
								formLogin.find('input[type="email"]')
										.toggleClass('has-error').next('span')
										.toggleClass('is-visible');
							});
					formSignup.find('input[type="submit"]').on('click',
							function(event) {
								event.preventDefault();
